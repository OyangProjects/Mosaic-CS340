...
for file in os.listdir('tilesets/starrail'):
  tile = Image.open('tilesets/starrail/' + file).convert('RGB')
  starrail_tiles.append(tile)
  average_color = np.array(tile).mean(axis=0).mean(axis=0)
  starrail_colors.append(average_color)

starrail_tree = spatial.KDTree(starrail_colors)

requests.put(os.getenv('FLASK_ADDRESS'), data={ 'name' : "Star Rail MMG", 'url' : os.getenv('STARRAIL_URL'), 'author' : 'oweny2', 'tileImageCount' : 60})

@app.route('/', methods=['GET'])
def GET_index():
  '''Route for '/' (frontend)'''
  return 'Starrail Mosaic'

@app.route('/makeMosaic', methods=['POST'])
def POST_makeMosaic():
  global starrail_tiles
  ...
  starrail_output = Image.new('RGB', (output_width, output_height))

  # Creating Mosaic

  for i in range(60):
    starrail_tiles[i] = starrail_tiles[i].resize((renderedTileSize, renderedTileSize))
  ...
  while(y_pixel < new_height):
    while(x_pixel < new_width):
      ...
      index = starrail_tree.query(average_color)[1]
      starrail_output.paste(starrail_tiles[index], (x_out, y_out))
  ...
  # starrail_output.save('output/starrail.' + fileFormat.lower() , fileFormat)

  output = BytesIO()
  starrail_output.save(output, format=fileFormat)
