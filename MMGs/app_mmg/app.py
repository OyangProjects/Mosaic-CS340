...
for file in os.listdir('tilesets/app_icons'):
  tile = Image.open('tilesets/app_icons/' + file).convert('RGB')
  app_tiles.append(tile)
  average_color = np.array(tile).mean(axis=0).mean(axis=0)
  app_colors.append(average_color)

app_tree = spatial.KDTree(app_colors)

requests.put(os.getenv('FLASK_ADDRESS'), data={ 'name' : "App Icons MMG", 'url' : os.getenv('APP_URL'), 'author' : 'oweny2', 'tileImageCount' : 60})
...
def POST_makeMosaic():
  ...
  app_output = Image.new('RGB', (output_width, output_height))

  # Creating Mosaic

  for i in range(60):
    app_tiles[i] = app_tiles[i].resize((renderedTileSize, renderedTileSize))
  ...
  while(y_pixel < new_height):
    while(x_pixel < new_width):
      ...
      index = app_tree.query(average_color)[1]
      app_output.paste(app_tiles[index], (x_out, y_out))
  ...
  # app_output.save('output/app.' + fileFormat.lower() , fileFormat)

  output = BytesIO()
  app_output.save(output, format=fileFormat)
